...
PRETRAINED_PATH = r"S:\Srikanth\VIT\Academics\pythonProject\Signals_project\functions_classifier.pth"
...
NUM_CLASSES = 11           # waveform type classification
NUM_PARAMS = 7             # regression outputs (e.g., m,c,a,b,..)
OUTPUT_SIZE = NUM_CLASSES + NUM_PARAMS

MODEL_PATH = os.path.join(TRAIN_DIR, "functions_classifier_updated.pth")
...
train_dataset = GraphDataset(os.path.join(TRAIN_DIR,"train_labels.csv"), TRAIN_DIR, transform=transform)
val_dataset   = GraphDataset(os.path.join(VAL_DIR,"val_labels.csv"), VAL_DIR, transform=transform)
...
print("✅ Pretrained backbone loaded, new fc layer initialized for current task")
...
def train_model(model, train_loader, val_loader, epochs):
    ...
    for epoch in range(epochs):
        ...
        for images, labels in train_loader:
            ...
            cls_outputs = outputs[:, :NUM_CLASSES]       # classification
    ...
    with torch.no_grad():
        for images, labels in val_loader:
            images, labels = images.to(DEVICE), labels.to(DEVICE)
            outputs = model(images)
            cls_outputs = outputs[:, :NUM_CLASSES]
            _, predicted = cls_outputs.max(1)
